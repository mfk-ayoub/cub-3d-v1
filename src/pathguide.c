...
/*   Updated: 2025/02/21 06:23:41 by ayel-mou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/cub3d.h"

#define MINIMAP_SCALE 0.25 // Scales minimap to 200x150 pixels (800 * 0.25 = 200, 600 * 0.25 = 150)

// Helper function to draw a small circle (approximated as a filled circle with radius 1 or 2)
void draw_circle(t_cub *cub, int center_x, int center_y, int radius, int color)
{
    for (int y = -radius; y <= radius; y++)
    {
        for (int x = -radius; x <= radius; x++)
        {
            if (x * x + y * y <= radius * radius) // Check if within circle radius
            {
...
int calculate_pixel_size(t_data *data)
{
    ...
    return (fmax(1, fmin(pixel_size_w, pixel_size_h))); // Ensure at least 1 pixel per tile for clarity
}
...
void put_pixel_map(t_cub *cub, int map_x, int map_y, int color)
{
    ...
    for (i = 0; i < cub->pixel_size; i++)
    {
        for (j = 0; j < cub->pixel_size; j++)
        {
            int new_x = map_x + j;
            int new_y = map_y + i;
            if (new_x >= 0 && new_x < (int)(WIDTH * MINIMAP_SCALE) && new_y >= 0 && new_y < (int)(HEIGHT * MINIMAP_SCALE))
...
void path_guide(t_cub *cub, t_data *data)
{
    ...
    // Step 1: Draw the player circle first (small white circle, radius 1 for a 3x3 or 2 for a 5x5)
    int player_x = (int)(cub->player_x / TILE_SIZE);
    int player_y = (int)(cub->player_y / TILE_SIZE);
    if (player_x >= 0 && player_x < data->col && player_y >= 0 && player_y < data->row)
    {
        int center_x = player_x * cub->pixel_size + (cub->pixel_size / 2);
        int center_y = player_y * cub->pixel_size + (cub->pixel_size / 2);
        draw_circle(cub, center_x, center_y, 1, 0xFFFFFF); // Small white circle for player (radius 1 for 3x3)
    }
    ...
    for (y = 0; y < data->row; y++)
    {
        for (x = 0; x < data->col; x++)
        {
            ...
            {
                if (data->map[y][x] == '1')
                    put_pixel_map(cub, x * cub->pixel_size, y * cub->pixel_size, 0x7F8C8D); // Gray for walls
                else if (data->map[y][x] == '0' || data->map[y][x] == 'P' || data->map[y][x] == 'N' || 
                         data->map[y][x] == 'S' || data->map[y][x] == 'E' || data->map[y][x] == 'W')
                    put_pixel_map(cub, x * cub->pixel_size, y * cub->pixel_size, 0x4D5656); // Dark gray for empty spaces
            }
        }
    }
}