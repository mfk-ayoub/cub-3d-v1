...
/*   Updated: 2025/02/21 06:20:42 by ayel-mou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	get_horizontal_intersection(t_cub *cub, t_rays *rays)
{
	...
	horizontal.direction = check_horizontal_intersection(horizontal.angle,
			&horizontal.y_step, &horizontal.inter_y);
	horizontal.inter_x = cub->player_x + (horizontal.inter_y - cub->player_y)
		/ tan(horizontal.angle);
	adjust_x_step(horizontal.angle, &horizontal.x_step);
	while (!check_if_wall(cub->data, horizontal.inter_x, horizontal.inter_y
	...
	rays->flag = 0;
	return (calculate_distance(horizontal.inter_x, cub->player_x,
			horizontal.inter_y, cub->player_y));
}
...
double	get_vertical_intersection(t_cub *cub, t_rays *rays)
{
	...
	vertical.direction = check_vertical_intersection(vertical.angle,
			&vertical.x_step, &vertical.inter_x);
	vertical.inter_y = cub->player_y + (vertical.inter_x - cub->player_x)
		* tan(vertical.angle);
	adjust_y_step(vertical.angle, &vertical.y_step);
	while (!check_if_wall(cub->data, vertical.inter_x - vertical.direction,
			vertical.inter_y))
	{
	...
	rays->flag = 1;
	return (calculate_distance(vertical.inter_x, cub->player_x,
			vertical.inter_y, cub->player_y));
}
